...
PreprocessCuda::PreprocessCuda(cudaStream_t stream) {
  ...
  unsigned int mask_size = params_.grid_z_size * params_.grid_y_size *
                           params_.grid_x_size * sizeof(unsigned int);

  unsigned int voxels_size = params_.grid_z_size * params_.grid_y_size *
                             params_.grid_x_size *
...
int PreprocessCuda::generateVoxels(float *points, unsigned int points_size,
  ...
  checkCudaErrors(generateVoxels_random_launch(
      ...
      grid_y_size, grid_x_size, mask_, voxels_, stream_));

  checkCudaErrors(generateBaseFeatures_launch(
      mask_, voxels_, grid_y_size, grid_x_size, pillar_num, voxel_features,
      voxel_num, voxel_idxs, stream_));
